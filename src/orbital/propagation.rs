...
mod tests {
    ...
    fn test_minutes_since_epoch_leap_year_boundaries() {
        ...
        assert!((minutes - expected_minutes).abs() < 1e-10,
                "Leap year boundary: expected {} minutes, got {}", expected_minutes, minutes);
        ...
        assert!((non_leap_minutes - expected_non_leap).abs() < 1e-10,
                "Non-leap year: expected {} minutes, got {}", expected_non_leap, non_leap_minutes);
    }
    ...
    fn test_minutes_since_epoch_century_boundaries() {
        ...
        assert!((minutes_century - expected_century_minutes).abs() < 1.0,
                "Century span: expected {} minutes, got {}", expected_century_minutes, minutes_century);
    }
    ...
    fn test_minutes_since_epoch_high_precision() {
        ...
        let sim_time = epoch + chrono::Duration::seconds(90) + chrono::Duration::nanoseconds(500_000_000);
        
        let minutes = minutes_since_epoch(sim_time, epoch);
        let expected = 1.5 + (500_000_000.0 / 1e9) / 60.0; // 1.5 + 0.5s in minutes
        
        assert!((minutes - expected).abs() < 1e-12,
                "High precision: expected {} minutes, got {}", expected, minutes);
        ...
        assert!((micro_minutes - expected_micro).abs() < 1e-15,
                "Microsecond precision: expected {} minutes, got {}", expected_micro, micro_minutes);
    }
    ...
    fn test_minutes_since_epoch_large_time_spans() {
        ...
        assert!(diff < 10.0 * 24.0 * 60.0,
                "54-year span should be approximately {} minutes, got {} (diff: {})",
                approx_minutes, minutes_54_years, diff);
        ...
        assert!(future_diff < 10.0 * 24.0 * 60.0,
                "Future 76-year span should be approximately {} minutes, got {}",
                approx_future, future_minutes);
    }
    ...
    fn test_minutes_since_epoch_negative_spans() {
        ...
        assert!((negative_minutes + 120.0).abs() < 1e-10,
                "Negative span: expected -120 minutes, got {}", negative_minutes);
        ...
        assert!((negative_frac + 1.5).abs() < 1e-10,
                "Negative fractional: expected -1.5 minutes, got {}", negative_frac);
    }
    ...
    fn test_minutes_since_epoch_same_time() {
        ...
        assert!(zero_minutes.abs() < 1e-15,
                "Same time should give 0 minutes, got {}", zero_minutes);
    }
    ...
    fn test_minutes_since_epoch_daylight_saving_transitions() {
        ...
        assert!((spring_minutes - 120.0).abs() < 1e-10,
                "Spring DST transition: expected 120 minutes, got {}", spring_minutes);
        ...
        // Should be exactly 2 hours = 120 minutes (UTC is not affected by DST)
        assert!((fall_minutes - 120.0).abs() < 1e-10,
                "Fall DST transition: expected 120 minutes, got {}", fall_minutes);
    }
    ...
    fn test_minutes_since_epoch_extreme_precision() {
        ...
        assert!((nano_minutes - expected_nano_minutes).abs() < 1e-18,
                "Nanosecond precision: expected {} minutes, got {}", expected_nano_minutes, nano_minutes);
        ...
        assert!((max_nano_minutes - expected_max_nano).abs() < 1e-15,
                "Max nanosecond precision: expected {} minutes, got {}", expected_max_nano, max_nano_minutes);
    }
    ...
    fn test_minutes_since_epoch_consistency_with_duration() {
        ...
        let duration_minutes = duration.num_seconds() as f64 / 60.0 +
                              (duration.subsec_nanos() as f64) / (60.0 * 1e9);
        
        assert!((our_minutes - duration_minutes).abs() < 1e-12,
                "Should match chrono::Duration calculation: our {} vs duration {}",
                our_minutes, duration_minutes);
    }
}
