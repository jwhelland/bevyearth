...
mod tests {
    ...
    fn test_advance_simulation_clock_extreme_time_scales() {
        ...
        assert!(year_diff >= 0, "Should advance time, got {} year difference", year_diff);
        
        // Check that time actually advanced significantly
        let time_diff = sim_time.current_utc - original_time;
        let seconds_diff = time_diff.num_seconds();
        assert!(seconds_diff > 1000000, "Should advance by many seconds, got {}", seconds_diff);
        ...
        assert!(slow_nanos > 0 && slow_nanos < 10000, "Slow advance should be small: {} ns", slow_nanos);
    }
    ...
    fn test_simulation_time_consistency_across_boundaries() {
        ...
        for (start_time, advance_sec, (exp_year, exp_month, exp_day, exp_hour, exp_min, exp_sec)) in test_cases {
            let mut sim_time = SimulationTime {
